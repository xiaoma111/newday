...
def studentquanxian(request):
    ...
    return render(request,'studentquanxian.html',{"queryname":queryname,"students":newstudents,"equipments":equip})
...
def teacherform(request):
    ...
    print("ID:", queryname)
    teacherall = yuyue.objects.filter(Q(ysid__steacher__tname__icontains=queryname),Q(ydate__gte=startTime),Q(ydate__lte=endTime),Q(isquxiao=False),Q(isqiandao=False))
...
def weekform(request):
    ...
    shebei1 = yuyue.objects.filter(Q(yeid__ename__icontains=queryname),Q(ydate__gte=startTime),Q(ydate__lte=endTime),Q(isquxiao=False),Q(isqiandao=False))
