...
class yuyue(models.Model):
    ...
    ytimestart=models.CharField('开始时间(输入格式为xx:xx)',max_length=30)
    shichang=models.DecimalField('时长',max_digits=5,decimal_places=1,default=1)
    yuyuebeizhu=models.CharField("备注",max_length=200,blank=True)
    shiyanneirong = models.CharField("实验内容",max_length=200,blank=True)
    ...
    kaishishijian=models.CharField('实际开始时间',max_length=200,blank=True)
    jieshushijian=models.CharField('实际结束时间',max_length=200,blank=True)
    ...
    def save(self):
        # print("instace0:", self.shichang)
        thisdate = self.ydate
        thisstarttime = self.ytimestart
        if (":" in thisstarttime):
            thisstarttime = int(thisstarttime.split(":")[0])
        else:

            thisstarttime = int(thisstarttime.split("：")[0])
        thisshichang = int(self.shichang)

        thisendtime = thisstarttime + thisshichang  # int型
        thisysid = self.ysid
        thisyeid = self.yeid

        yuyues = yuyue.objects.filter(Q(isquxiao=False), Q(ydate=thisdate),Q(yeid=thisyeid)).order_by('ydate')
        # print("yuyues:", yuyues)
        self.ytimestart = str(thisstarttime) + "：00"

        tt=0
        for yuyueinfo1 in yuyues:
            herestarttime = yuyueinfo1.ytimestart
            hereysid = yuyueinfo1.ysid
            hereyeid = yuyueinfo1.yeid

            herestarttime = int(herestarttime.split("：")[0])
            hereshichang = int(yuyueinfo1.shichang)
            hereendtime = herestarttime + hereshichang

            if (self.id == yuyueinfo1.id):
                super(yuyue, self).save()
                print("直接存储")
                tt=1
                break

            elif not (hereendtime<=thisstarttime or herestarttime>=thisendtime):
        ...
        if(tt!=1):
            ...
            for yuyueinfo in yuyues:
                herestarttime = yuyueinfo.ytimestart
                hereysid = yuyueinfo.ysid
                hereyeid = yuyueinfo.yeid

                herestarttime = int(herestarttime.split("：")[0])
                hereshichang = int(yuyueinfo.shichang)
                hereendtime = herestarttime + hereshichang

                if (hereendtime == thisstarttime and hereyeid == thisyeid and hereysid == thisysid):

                    self.ytimestart = str(herestarttime) + "：00"
                    self.shichang = self.shichang + hereshichang
                    yuyueinfo.delete()

                elif (herestarttime == thisendtime and hereyeid == thisyeid and hereysid == thisysid):
                    self.shichang = self.shichang + hereshichang
                    yuyueinfo.delete()

            # print("instace1:", self.shichang)
            #
            # print("self:",self.yeid,self.ysid)
...
class guanliyuan(models.Model):
    ...
    gname=models.CharField('姓名',max_length=20)
    gphone=models.CharField('联系方式',max_length=20)
    ischoose=models.BooleanField('选中',default=False)
    def __str__(self):
        return self.gname