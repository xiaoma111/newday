...
from .models import teacher,student,equipment,quanxian,yuyue,guanliyuan
import json
...
def studentshenqing(request):
    ...
    if((len(equipments) // 3)==0):
        equip.append(equipments)
...
def foradmin2(request):
    ...
    # if str(user) == "AnonymousUser":
        ...
        for item in shebeiall:
            ...
            shichangtongji.append({"eid": eid, "ename": ename,  "eshiyanshi": eshiyanshi,"eguanliyuan":eguanliyuan,
                           "ezhuangtai": ezhuangtai,})

        shichangtongji.sort(key=lambda x: (x["ename"]))
        ...
        guanliyuans=guanliyuan.objects.all().order_by('-ischoose','gname')



        return render(request,'foradmin2.html',{"number1":number1,'students':students,'equipments':equip,"shichang":shichang1,"weiyueinfos":weiyueinfos1,"guanliyuans":guanliyuans})
...
def shebeiform(request):
    if not request.user.is_authenticated():
        ...
        shebei1 = yuyue.objects.filter(Q(yeid__ename__icontains=queryname), Q(ydate__gte=startTime),
                                       Q(ydate__lte=endTime),Q(isquxiao=False),Q(isqiandao=False))
        shebeis = shebei1.values_list("yeid__eid", "yeid__ename","ysid__sname","ydate","kaishishijian","jieshushijian",'shiyanneirong',"shiyanfankui").order_by('yeid__eid','ydate',"ytimestart")
        ...
        for index ,row in enumerate(shebeis):
            ...
            result.append({"eid": eid, "ename": ename, "sname": sname,"ydate":ydate,"kaishishijian":kaishishijian,"jieshushijian":jieshushijian,"shiyanneirong":shiyanneirong,"shiyanfankui":shiyanfankui})
...
def shebeizhuangtai(request):
    if not request.user.is_authenticated():
        ...
        for item in shebeiall:
            ...
            result.append({"eid": eid, "ename": ename, "eguanliyuan": eguanliyuan,"eshiyanshi":eshiyanshi,"ezhuangtai":ezhuangtai})



        result.sort(key=lambda x:(x["ename"]))
        print("result:", result)
...
def getguanliyuan(request):
    jsonstring = json.loads(request.body.decode())
    eid = jsonstring["id"]
    guanliyuanck=guanliyuan.objects.filter(ischoose=True)
    for item in guanliyuanck:
        item.ischoose=False
        item.save()
    studentpk = guanliyuan.objects.filter(id=eid)[0]
    studentpk.ischoose=True
    studentpk.save()
